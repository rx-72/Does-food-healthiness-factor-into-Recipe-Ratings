...
The correlation trendline (in red) does demonstrate that the correlation is below 0. There's also potential interest in 2 factors. Firstly is if the rating is an integer, we have a strong distribution of low and high calories. This is most viewable in rating 0, who despite it's rating, still has a column of calories with a similar distribution to rating 5. Another major interest is the amount of points in the higher bins of ratings (between 3.5 - 5). This aligns with the idea that we our whisker plot focused greater around higher values past the first quartile. In regards to our hypothesis question, depending on where our level of healthy calories, the plot implies that if above around 1500 calories, we'll have different distributions in ratings. This is highly due to the large amount of values focused around the 4-5 bin of ratings that are situatied around range 1000. Hence this scatterplot may not demonstrate a correlation between the two columns but it will be highly useful in predicting whether a healthy calory range may prove dependent on ratings since we can clearly see the borderline where we may have either similar distribution of very different distributions between above or below a specific calory level. (example y=500 calories, may indicate similar distribution of values of ratings for above values and below values of y, while y=2000 columns may have very different values since we have a lot more values beneath 200 calories and the distribution is then more clearly different in regards to the range of ratings.)
