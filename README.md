...
To conclude, we can say at the highest level of possible healthiness in nutritional varaibles, there is evidence that certain nutritional variables may play a role in determining the average rating of a recipe (though not evidence for all of them). If we wanted to divide further into this study, we could try using a lower test variables (we could do a calory set of 500 since breakfast meals tend to be closer to 500 calories for example) to determine how low we can go before a variable no longer becomes significant to the average rating, or find out when a variable could become significant to ratings if it wasn't already. Similarly we could also further explore compound effects such as how multiple of these factors could play a role together onto the column of average recipe ratings (since we only tested on one to one column relationships.) This is especially important if nutritional factors by themselves aren't significant but become significant when compounded with another factor. That said, since we do know there existed hypthosis tests we ended up rejecting the null hypothesis with, we now have further data to answer our question but also expand on it and investigate further on these possible relationships with the aforementioend methods.
